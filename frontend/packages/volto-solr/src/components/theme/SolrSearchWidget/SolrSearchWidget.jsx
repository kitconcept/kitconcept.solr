...
const SolrSearchAutosuggestRaw = (props) => {
  ...
  const getInputRef = () =>
    props.inputRef !== undefined ? props.inputRef : inputRef;
  ...
  const enhancedOnChange = (evt) => {
    // When calling onChange, also support the value directly
    // in a dictionary as second parameter. Make sure that an empty
    // value is always an empty string, never undefined.
    props.onChange(evt, {
  ...
  const storeInputReference = (autosuggest) => {
    if (autosuggest) {
      // Take away the focus from the input field,
      // this is instrumental in closing the dropdown
      getInputRef().current = autosuggest.input;
